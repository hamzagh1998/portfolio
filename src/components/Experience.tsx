    ...
    <section id="experience" className="container">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        {experiences.map((exp, index) => (
          <div key={index} className="glass card-hover" style={{ padding: 'var(--card-padding, 2.5rem)', transition: 'var(--transition)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem', flexWrap: 'wrap', gap: '1rem' }}>
              <div>
                <h3 style={{ fontSize: '1.5rem', color: 'var(--accent-cyan)' }}>{exp.role}</h3>
                <h4 style={{ fontSize: '1.125rem', color: 'var(--text-main)' }}>{exp.company}</h4>
              </div>
